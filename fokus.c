/*
 * fokus - A minimalist terminal‐based focus timer and stopwatch with daily
 logging, built on ncurses.
...
const char *config_header = "# fokus config file\n"
                            "\n"
...
int read_or_create_config(int *fps) {
  ...
  *fps = 30;
  ...
  if (!f) {
    ...
    if (!has_default_timer || !has_fps) {
      fseek(f, 0, SEEK_END);
      if (!has_default_timer)
        fprintf(f,
                "\n"
                "# Default countdown timer duration (in minutes)\n"
                "# Must be between 1 and 999\n"
...
void init_log_path() {
  ...
  snprintf(lock_filepath, sizeof(lock_filepath), "/tmp/fokus-%d.lock",
           getuid());
  snprintf(config_filepath, sizeof(config_filepath), "%s/fokus.conf",
           config_dir);
...
void draw_slider(int top, int height, int total_lines, int offset) {
  ...
  int slider_pos =
      (int)((float)offset / (total_lines - height) * (height - slider_height));
  if (slider_pos < 0)
    slider_pos = 0;
...
void draw_logs(int rows, int cols, int offset) {
  ...
  attron(COLOR_PAIR(1));
  mvprintw(frame_top + 1, frame_left + (frame_w - (int)strlen(hdr)) / 2, "%s",
           hdr);
  attroff(COLOR_PAIR(1));
  ...
  for (int i = 0; i < display_count; i++) {
    char ln[64];
    snprintf(ln, sizeof(ln), "%-12s %6d", E[offset + i].date,
             E[offset + i].minutes);
    mvprintw(frame_top + 2 + i, frame_left + (frame_w - (int)strlen(ln)) / 2,
             "%s", ln);
  }
...
void draw_footer(int rows, int cols) {
  const char *full_ft = "[space] Start/Reset  [q] Quit  [h]/[l] Change Page  "
                        "[j]/[k] Adjust/Scroll";
...
int main() {
  ...
  if (today[0] == '\0') {
    snprintf(today, sizeof(today), "%04d-%02d-%02d", tm->tm_year + 1900,
  ...
  while (1) {
    ...
    int dms =
        (cur.tv_sec - last.tv_sec) * 1000 + (cur.tv_usec - last.tv_usec) / 1000;
    if (dms >= 10) {
      now = time(NULL);
      tm = localtime(&now);
      char cd[11];
      snprintf(cd, sizeof(cd), "%04d-%02d-%02d", tm->tm_year + 1900,
               tm->tm_mon + 1, tm->tm_mday);
    ...
    if (page == 1) {
      ...
      if (!run_sw && !run_tm)
        draw_minutes(today_m, rows, cols);
    } else if (page == 2) {
